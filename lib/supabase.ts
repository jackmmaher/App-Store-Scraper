...
export interface WireframeData {
  version: string;
  screens: Record<string, WireframeScreen>;
  settings: {
    deviceFrame: string;
    gridSize: number;
  };
...
export async function createConcept(input: CreateConceptInput): Promise<AppConcept | null> {
  const { name, description, linked_project_ids } = input;
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      wireframe_data: initialWireframeData,
      export_history: [],
    })
    .select()
    .single();
...
// Get all concepts
export async function getConcepts(): Promise<AppConcept[]> {
  const { data, error } = await supabase
    .from('app_concepts')
    .select('*')
    .order('updated_at', { ascending: false });
...
// Get multiple projects by IDs (for concept insights)
export async function getProjectsByIds(ids: string[]): Promise<AppProject[]> {
